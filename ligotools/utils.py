...
whiten: function to whiten data
write_wavfile: function to keep the data within integer limits, and write to wavfile
...
def make_the_plots(det, strain_L1_whitenbp, strain_H1_whitenbp, template_match, time, timemax, tevent, SNR, eventname, plottype, template_fft, datafreq, d_eff, freqs, data_psd, fs):
    ...
    plt.xlabel('Time since {0:.4f}'.format(timemax))
    plt.legend(loc='upper left')
    plt.savefig(figure_path+eventname+"_"+det+"_SNR."+plottype)
    ...
    plt.savefig(figure_path+eventname+"_"+det+"_matchtime."+plottype)
    ...
    plt.savefig(figure_path+eventname+"_"+det+"_matchfreq."+plottype)